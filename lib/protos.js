    ...
    case 'script':
      ...
      el = loadScript(attrs, function(err){
        ...
        timeout = setTimeout(function(){
          var err = new Error('script error, cannot load "' + src + '"');
          err.timeout = true;
          clearTimeout(tid);
          self.debug('load timeout of 3s exceeded');
          fn(err);
        }, 3000);

        // retry
        function retry(){
          if (!self.loaded()) return setTimeout(retry, 100);
          clearTimeout(timeout);
          clearTimeout(tid);
          self.debug('loaded after retrying');
          fn();
        }
